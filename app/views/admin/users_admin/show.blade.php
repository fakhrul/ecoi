...
@stop
<?
define("PAGETITLE", " | View Admin");

?>
@section('content')
<div class="clearfix">
	<div class="visible-md-block visible-lg-block">
		{{ Form::open(array('route' => array('admin.users_admin.destroy', $user->id), 'method' => 'delete')) }}
		<a id="back_to_user" type="button" class="btn btn-primary" href="{{ URL::to('admin/users_admin') }}"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;Back</a>
		@if (!$user->deleted_at)
		<a type="button" class="btn btn-primary" href="{{ URL::to('admin/users_admin/' . $user->id) .'/edit' }}"><span class="glyphicon glyphicon-pencil"></span>&nbsp;Edit</a>
		<button id="delete_user" type="submit" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span>&nbsp;Delete</button>
		@endif
		@if ($user->deleted_at)
		<a id="restore_user" type="button" class="btn btn-primary" href="{{ URL::to('admin/users_admin/restore?id=' . $user->id) }}"><span class="glyphicon glyphicon-repeat"></span>&nbsp;Restore</a>
		@endif
		{{ Form::close() }}
	...
	<div class="visible-xs-block visible-sm-block dropdown">
		...
		<ul class="dropdown-menu" role="menu">
			<li><a id="back_to_user" href="{{ URL::to('admin/users_admin') }}"><span class="glyphicon glyphicon-arrow-left"></span> Back</a></li>
			@if (!$user->deleted_at)
			<li><a href="{{ URL::to('admin/users_admin/' . $user->id) .'/edit' }}"><span class="glyphicon glyphicon-pencil"></span> Edit</a></li>
			...
			@endif
			@if ($user->deleted_at)
			<li><a id="restore_user" href="{{ URL::to('admin/users_admin/restore?id=' . $user->id) }}"><span class="glyphicon glyphicon-repeat"></span> Restore</a></li>
			@endif
		</ul>
		{{ Form::close() }}
	</div>
