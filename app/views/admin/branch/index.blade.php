...
@endif
	...
	<div class="clearfix">
		<div class="visible-md-block visible-lg-block">
		  	...
		  	<a type="button" class="btn btn-primary" href="{{ URL::to('admin/branch/create') }}"><span class="glyphicon glyphicon-plus"></span>&nbsp;Add</a>
		...
		<div class="visible-xs-block visible-sm-block dropdown">
			...
			<ul class="dropdown-menu" role="menu">
				...
				<li><a href="{{ URL::to('admin/branch/create') }}"><span class="glyphicon glyphicon-plus"></span> Add</a></li>
			</ul>
		</div>
	</div>
			...
			<table class="table table-hover table-striped table-bordered">
				<thead>
					<tr>
						...
						<th class="{{($sort=='channel_id')? (($sort_order=='desc')? 'sort_desc' : 'sort_asc') : 'sort' }} pointer" value="channel_id">Channel</td>		
						<th>Action</td>
					</tr>
				</thead>
				<tbody>
				@foreach($list as $key => $value)
					...
					@if (false and allowed('admin.branch.destroy'))
					<!-- delete the nerd (uses the destroy method DESTROY /nerds/{id} -->
					<!-- we will add this later since its a little more complicated than the other two buttons -->
					{{ Form::open(array('url' => 'admin/branch/' . $value->id)) }}
					{{ Form::hidden('_method', 'DELETE') }}
					{{ Form::submit('Delete', array('class' => 'btn btn-warning pull-right')) }}
					{{ Form::close() }}
					@endif				

					@if (allowed('admin.branch.show'))
					<!-- show the nerd (uses the show method found at GET /nerds/{id} -->
					<a href="{{ URL::to('admin/branch/' . $value->id) }}">View</a>
					@endif
					|
					@if (allowed('admin.branch.edit'))
					<!-- edit this nerd (uses the edit method found at GET /nerds/{id}/edit -->
					<a href="{{ URL::to('admin/branch/' . $value->id . '/edit') }}">Edit</a>
					@endif
						</td>
					</tr>
				@endforeach
...
{{ $list->appends(array('paging' => $list->getPerPage(), 'sort' => Input::get('sort'), 'sort_order' => Input::get('sort_order'), 'channel_id' => Input::get('channel_id'), 'name' => Input::get('name')))->links(); }}
	    	...
	    	<div class="modal-body">
				<form id="form_search" class="form-horizontal" role="form" method="get">
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-5">
					    	{{ Form::select('channel_id',$channels, Input::get('channel_id'), array('class' => 'form-control')) }}
					    </div>
			  		</div>
			  		<div class="form-group">
			    		<label for="name" class="col-sm-2 control-label">Branch Name</label>
					    <div class="col-sm-5">
					    	<input type="text" class="form-control" id="name" name="name" value="{{Input::get('name')}}" placeholder="">
					    </div>
			  		</div>
