...
@if(isset($stations))
    <div class="clearfix">
        ...
        @if (allowed('admin.station.create'))
            <a type="button" class="btn btn-primary" href="{{ URL::to('admin/station/create') }}"><span class="glyphicon glyphicon-plus"></span>&nbsp;Add</a>
        @endif
    	...
    	<div class="visible-xs-block visible-sm-block dropdown">
            ...
            @if (allowed('admin.station.create'))
                <li><a href="{{ URL::to('admin/station/create') }}"><span class="glyphicon glyphicon-plus"></span> Add</a></li>
            @endif
	...
	{{ Form::open([ "id"=>"form_export", "name"=>"form_export", "url" => "admin/station/export"]) }}
    	    ...
    	    <table class="table table-hover table-striped table-bordered">
    			...
    			<tbody>
    				...
    				<tr>
    					...
    					<td>
            				@if (false and allowed('admin.station.destroy'))
            				<!-- delete the nerd (uses the destroy method DESTROY /nerds/{id} -->
            				<!-- we will add this later since its a little more complicated than the other two buttons (if required)-->
            				{{-- Form::open(array('url' => 'admin/station/' . $value->id)) --}}
            				{{-- Form::hidden('_method', 'DELETE') --}}
            				{{-- Form::submit('Delete', array('class' => 'btn btn-warning pull-right')) --}}
            				{{-- Form::close() --}}
            				@endif				
            
            				@if (allowed('admin.station.show'))
            				<!-- show the nerd (uses the show method found at GET /nerds/{id} -->
            				<a href="{{ URL::to('admin/station/' . $value->id) }}">View</a>
            				@endif
            				 
            				@if (allowed('admin.station.edit'))
                            |                                                        
            				<!-- edit this nerd (uses the edit method found at GET /nerds/{id}/edit -->
            				<a href="{{ URL::to('admin/station/' . $value->id . '/edit') }}">Edit</a>
            				@endif
    					</td> 
    				</tr>
                    @endforeach
