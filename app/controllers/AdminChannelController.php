...
class AdminChannelController extends BaseController {
	...
	public function index()
	{
		...
		}else { $channels = Channel::whereIn('brand_id',(Auth::user()->brands()->lists('brand_id')))->orderBy($sort, $sort_order); }
    	...
    	$breadcrumbs = array('Channel' => 'admin/channels' , 'Manage Channel' => 'admin/channels');

		return View::make('admin.channels.index')->with('breadcrumbs',$breadcrumbs)->with("sort", $sort)->with("sort_order",$sort_order)->with('list', $channels)->with('upline_options', $upline_options);
	}
	...
	public function store()
	{
		...
		if ($validator->fails()) {
			...
			$channel->brand_id 	= Input::get('brand_id');			
...
#				}
	...
	public function export()
	{
		...
		}else { $channels = Channel::whereIn('brand_id',(Auth::user()->brands()->lists('brand_id')))->orderBy('created_at', 'asc'); }
