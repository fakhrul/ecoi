...
class StationController extends BaseController {
	...
	public function index(){
        ...
        $station_states = State::has('Stations')->lists('name', 'id');
    	...
    	$breadcrumbs = array('Station' => 'station', 'Manage Station' => 'station');
        
		return View::make('station.index',compact('sort', 'sort_order'))->with('breadcrumbs',$breadcrumbs)->with('station_states', $station_states)->with('stations', $stations); 	
	}
	...
	public function create(){//
		...
		$breadcrumbs = array('Station' => 'station', 'Add Station' => 'station/create');

		return View::make('station.create', $data)->with('breadcrumbs',$breadcrumbs)->with('states',$states)->with('types',$types);
	}
	...
	public function store(){
		...
		// process the login
		if ($validator->fails()) {
			return Redirect::to('station/create')->withErrors($validator)->withInput(Input::except('password'));
		} else {
            ...
            //Attach the station to super Admin and nazer and anthony
            $user_station[1] = array('status' => 1, 'created_by' => Auth::user()->id,  'updated_by' => Auth::user()->id) ;
            $user_station[2] = array('status' => 1, 'created_by' => Auth::user()->id,  'updated_by' => Auth::user()->id) ;
            $user_station[3] = array('status' => 1, 'created_by' => Auth::user()->id,  'updated_by' => Auth::user()->id) ;
            if(Auth::user()->id != 1){
			...
			return Redirect::to('station/'.$station->id);
		}
	}
	...
	public function show($id){
		...
		if(!empty($station)) {
			$breadcrumbs = array('User' => 'users', 'View User' => 'users');	
			// show the view and pass the nerd to it
			return View::make('station.show')->with('breadcrumbs',$breadcrumbs)->with('station', $station);
		}else{	
	...
	public function edit($id){//
		...
		$breadcrumbs = array('Station' => 'station', 'Edit Station' => 'station/edit');

		return View::make('station.edit', $data)->with('breadcrumbs',$breadcrumbs)->with('states',$states)->with('station',$station)->with('types',$types)
	...
	public function update($id){
		...
		// process the login
		if ($validator->fails()) {
			return Redirect::to('station/' . $id . '/edit')->withErrors($validator)->withInput(Input::except('password'));
		} else {
			$station 				    = Station::find($id);
			$station->station_code 		= Input::get('station_code');
			$station->station_name    	= Input::get('station_name');
